    ...
    const newLayer = this._wrapper
      ...
      .then((d) => {
        if (layer.geoJson) {
          ...
          this.getDataFeatures(d, layer.geoJson).then(
            (features) => ((d as any).features = features)
          );
        }
        return d;
      });
  ...
  updateGeoJson(layer: AgmDataLayer, geoJson: object | string) {
    this._layers.get(layer).then((l) => {
      ...
      this.getDataFeatures(l, geoJson).then(
        (features) => ((l as any).features = features)
      );
    });
  }

  setDataOptions(layer: AgmDataLayer, options: any) {
    this._layers.get(layer).then((l) => {
